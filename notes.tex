...
\label{sec:orgfe9710e}

\subsection{Examples of metric spaces}
\label{sec:org6781940}

\subsubsection{The set of continuous functions \(C[a, b]\)}
\label{sec:org71dc62d}
...
\label{sec:org5640b2a}
\subsubsection{Definition of a norm}
\label{sec:org127a6e9}
\subsubsection{Examples}
\label{sec:org2e6e041}
\begin{enumerate}
\item \(\R^n\)
...
\label{sec:org818a99a}
\subsection{Remark}
\label{sec:orgbcd2a1e}
...
\label{sec:org75c8e95}
\section{Lecture 5 \textit{<2018-11-01 Thu>}}
\label{sec:org1235fa2}
\subsection{Review}
\label{sec:org8374644}
...
\(Y\).
\subsubsection{Proof}
\label{sec:org027e075}
Proof is easy.
\subsection{Corollary}
\label{sec:orged7d77e}
...
\label{sec:orge7da265}
\subsection{Example}
\label{sec:org3954ede}
...
\label{sec:org8918107}
\subsection{About finite linear operators}
\label{sec:orge6ab15c}
...
\label{sec:org51ebb82}
\subsection{Koopman operator}
\label{sec:org5db3536}
\(Kf = f\circ I\)
...
\label{sec:orgd707c03}
\subsection{About \(l^p\) and \(l^q\)}
\label{sec:org7c5e412}
...
\label{sec:org124510b}
\subsection{Theorem}
\label{sec:orgc16bab9}
...
\label{sec:org5c861c0}
\subsection{Adjoint operators}
\label{sec:org802bd36}
...
\label{sec:orgf70ba94}
\subsubsection{Example 1}
\label{sec:org9d4434a}
...
\label{sec:org5c28157}
\subsection{Continuation on the example}
\label{sec:orgc1c4be3}
...
\label{sec:org1d5f848}
\subsection{Planchevel's theorem}
\label{sec:org6845646}
...
\label{sec:org1dad129}
\section{Lecture 23}
\label{sec:org09f4bdb}
\subsection{Example}
\label{sec:org4be4acc}
...
\section{Lecture 24}
\label{sec:org0ef702f}
\subsection{An example about a question}
\label{sec:orgb2437db}
...
We can now derive the \textbf{uniform boundedness theorem} which states that if a
...
\section{Lecture 26}
\label{sec:orgd386375}
\subsection{Repetition}
\label{sec:org8ab2ada}
\begin{enumerate}
\item Continuous map: open set set inverted to open. Similarly for closed
\item Image of a compact set is compact
\item continuous function maps Connected sets to connected set.
\item Definition of disconnected. We can write \(X\) and \(A \cup B\) where \(A\) and
\(B\) are open and they are disjoint.
\item Some counter examples in the other direction.
\begin{enumerate}
\item Preimages of connected sets need to not be connected. \(f(x) = \vert x
         \vert\), then \(f^{-1}([1, 2]) = [-2, -1] \cup [1, 2]\).
\item \(f(x) = \frac{1}{1 + x^2}\)
\end{enumerate}
\end{enumerate}
\subsection{Theorem}
\label{sec:org95c9346}
Let \(X, Y\) be normed spaces and \(T \colon X \rightarrow Y\) be a linear
mapping. Then the following statements are equivalent.

\begin{enumerate}
\item \(T\) is open.
\item \(T\) maps open balls around \(0\) to neighbourhoods of \(0\), to be more
precise. \(U_r = \{x \in X \colon \Vert x \Vert < r\}\) and \(V_\varepsilon =
      \{y \in Y \colon \Vert y \Vert < \varepsilon\}\)
\item There was something that I missed.
\end{enumerate}
\subsection{Example}
\label{sec:org81ae0a1}
...
\textbf{This completes the fundamental theorem chapter}
\subsection{Spectral theory}
\label{sec:org650b141}
...
Some properties:
\begin{enumerate}
\item Every compact linear operator \(T\) is bounded and thus continuous (consider
\(U = \{x \in X \vert \Vert x \Vert = 1\}\), \(U\) is bounded and since \(T\) is
by definition compact, \(\overlin{T(U)}\) is compact and hence bounded.
\item \(T\) is compact if and only if it maps every bounded sequence \(x_n\) in \(X\)
onto a sequence \((Tx_n)\) in \(Y\) which has a convergent subsequence.
\item The compact linear operators from \(X\) to \(Y\) form a vector space, i.e.,
\(T_1 + T_2\) is compact if \(T_1, T_2\) are compact.
\end{enumerate}
\subsection{Definition}
\label{sec:org54e2b91}
