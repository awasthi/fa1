...
** Linear functionals
   ...
   $\alpa$, is easy to write $(f_1 + f_2)(x) = f_1(x) + f_2(x)$ etc. This part
   is obvious.
...
** About finite linear operators
   ...
   determined by knowing the coefficients $\gamm_{ji}$, which can be written in
   matrix form as
...
** Koopman operator
   ...
   \mu)f_3] + \alpha_3 \lambda^2 f_3 = \lapha_1\lambda f_1 + \alpha_2 \mu f_2 +
